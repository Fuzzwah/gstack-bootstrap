...
| `{{PREAMBLE}}` | `gen-skill-docs.ts` | Startup block: session tracking, proactive behavior, AskUserQuestion format |
...
2. **Proactive behavior** — reads proactive config. When false, skills only run when explicitly invoked.
